...
import { readableStreamToString, VisierProfileManagementError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierProfileManagementError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierProfileManagementError(e, e.response?.data)
                }
                if (innerError instanceof VisierProfileManagementError) {
                    // Got "VisierProfileManagementError" from the above try block
                    throw innerError;
                }
